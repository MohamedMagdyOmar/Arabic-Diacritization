...
def load_data():
    ...
    for each_sentence_number in sentence_numbers:
        ...
        training_sequence_list.append(x)
        training_padded_output.append(y)

    training_padded_input, vocabulary, vocabulary_inv = convert_input_to_vocab(training_sequence_list)
    training_padded_output = numpy.array(list(chain(*training_padded_output)))
    ...
    for each_sentence_number in sentence_numbers:
        ...
        testing_sequence_list.append(x)
        testing_padded_output.append(y)

    sequence_list = numpy.array(testing_sequence_list)
    sentences = list(chain(*sequence_list))
    testing_padded_input = dp.build_input_data2(sentences, vocabulary)
    testing_padded_output = numpy.array(list(chain(*testing_padded_output)))

    return training_padded_input, training_padded_output, vocabulary, vocabulary_inv, testing_padded_input, testing_padded_output
...
if __name__ == "__main__":
    ...
    # output dim
    embedding_vector_length = 52

    model = Sequential()

    model.add(Embedding(vocabulary_size, embedding_vector_length, input_length=sequence_length, batch_input_shape=(1, X_train.shape[1], embedding_vector_length)))
    print(model.summary())
    model.add(Bidirectional(LSTM(256, dropout=0.2, recurrent_dropout=0.2, return_sequences=True, stateful=True)))
    model.add(Bidirectional(LSTM(256, dropout=0.2, recurrent_dropout=0.2, return_sequences=True, stateful=True)))
    ...
    for i in range(10):
        model.fit(X_train, y_train, validation_data=(X_test, y_test), callbacks=callbacks_list, epochs=1, batch_size=1,
