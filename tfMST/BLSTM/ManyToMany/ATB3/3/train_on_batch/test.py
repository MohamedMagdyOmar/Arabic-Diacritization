...
def load_testing_data():
    ...
    return input_testing_padded, output_testing_padded, vocabulary, vocabulary_inv, testing_words, \
...
if __name__ == "__main__":
    ...
    model = load_model('tmpofi76_lh.h5')
    print(model.summary())
    nn_indices = []
    expected_indices = []
    prediction = []
    for x in X_test:
        x = x[numpy.newaxis, :]
        prediction.append(model.predict_on_batch(x))


    #prediction = numpy.array(prediction)
    for each_sequence in prediction:
        each_sequence = np.reshape(each_sequence, (each_sequence.shape[1], 50))
