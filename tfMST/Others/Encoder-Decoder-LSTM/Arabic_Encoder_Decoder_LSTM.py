...
print('Number of samples:', len(list_of_training_sentence_numbers))
...
encoder_input_data = np.zeros(
    (len(list_of_training_sentence_numbers), max_encoder_seq_length, num_encoder_tokens),
    dtype='float32')
decoder_input_data = np.zeros(
    (len(list_of_training_sentence_numbers), max_decoder_seq_length, num_decoder_tokens),
    dtype='float32')
...
for each_sentence_number in list_of_training_sentence_numbers:
    ...
    end_range = start_range + number_of_occurrence
    input_text = input_texts[start_range: end_range]
    target_text = target_texts[start_range: end_range]
    start_range = end_range
    print("we are processing sentence number:", each_sentence_number)
    for t, (input_char, target_char) in enumerate(zip(input_text, target_text)):
        try:
