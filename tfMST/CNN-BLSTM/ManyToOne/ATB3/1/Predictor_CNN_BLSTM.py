...
def load_testing_data():
    ...
    #testing_dataset = DBHelperMethod.load_dataset_by_type_and_sentence_number_for_testing_purpose("testing", 12)
    ...
    y = dp.load_nn_labels_dataset_string_For_ATB(testing_dataset[:, [0, 1]])
    sent_num, sen_len = dp.load_nn_seq_lengths(testing_dataset[:, [3]])
    sentences_padded_testing = dp.pad_sentences2(x_testing, sen_len, req_char_index, window_size)
    ...
    return padded_sent_testing, y, vocabulary, vocabulary_inv, testing_words, input_testing_letters, op_testing_letters,\
...
def get_undiac_words_for_selected_sentence(list_of_all_words_and_sent_num, sentence_number):
    c = list_of_all_words_and_sent_num[:, 1]
    list_of_undiac_words_and_sent = list_of_all_words_and_sent_num[
        list(np.where(c == str(sentence_number))[0])]
...
if __name__ == "__main__":
    ...
    model = load_model('weights.003-0.9038.hdf5')
    print(model.summary())
    ...
    for sentence_number in list_of_sentence_numbers:
        ...
        c = all_sentences[:, 1]
        indices_of_selected_sentence = np.where(c == str(sentence_number))
        ...
        error.append(DERCalculationHelperMethod.get_diacritization_error_version_2(master_object,
        ...
        error_without_last_letter.append(DERCalculationHelperMethod.
                                         ...
                                         (master_object, sentence_number, selected_sentence1))
        '''
