...
def get_input_and_target_data(type_of_dataset):
    ...
    list_of_input_character_query = "select * from UnDiacOneHotEncoding"
...
def create_netcdf_input(data, one_hot_data):
    start_time = datetime.datetime.now()
    search_counter = 0
    ...
    for eachItem in range(0, len(data)):
        input_character = data[eachItem][0]
        un_diacritized_word = data[eachItem][5]
        flag = True
        while flag:
            if one_hot_data[search_counter][1] == input_character:
                flag = False
                input_character_one_hot_encoding = map(int, list(str(one_hot_data[search_counter][2])))
                try:
                    if input_character == 'space' or input_character == 'eos' or input_character == 'bos':
                        input_character_one_hot_encoding.append(0)
                    elif un_diacritized_word != data[(eachItem + 1)][5]:
                        input_character_one_hot_encoding.append(1)
                    else:
                        input_character_one_hot_encoding.append(0)
                except:
                ...
                search_counter = 0
                netcdf_input.append(np.array(input_character_one_hot_encoding))
            else:
                search_counter += 1

    end_time = datetime.datetime.now()
    print "createNetCDFInput takes : ", end_time - start_time
