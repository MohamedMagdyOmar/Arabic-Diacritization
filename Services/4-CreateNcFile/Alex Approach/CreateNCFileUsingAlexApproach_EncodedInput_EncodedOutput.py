...
def create_netcdf_input(input_data):
    space = u'0000000000000000'
    start_time = datetime.datetime.now()
    netcdf_input_class = []
    for each_item in input_data:
        if each_item[0] == 'bos' or each_item[0] == 'eos' or each_item[0] == 'space':
            netcdf_input_class.append(np.array(map(int, list(str(space)))))
        else:
            netcdf_input_class.append(np.array(map(int, list(str(each_item[0])))))
...
if __name__ == "__main__":
    ...
    NetCDFInput = create_netcdf_input(InputData)
